...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<div class="post-content fullwidth clearfix">
		...
		<?php if($type=='slider') { 
		$category=MthemeCore::getPostMeta(get_the_ID(),"post_slider_cat","");
		echo do_shortcode('[carousel_slider category="'.$category.'"]'); ?>
		<?php  } elseif($type=='audio') { 
		$url=MthemeCore::getPostMeta(get_the_ID(),"post_url","https://api.soundcloud.com/tracks/145175216"); ?>
		...
		$url=MthemeCore::getPostMeta(get_the_ID(),"post_url",CHILD_URI."site/video.mp4");
		?>
		...
		$url=MthemeCore::getPostMeta(get_the_ID(),"post_url","http://player.vimeo.com/video/75976293");
		?>
		<div class="video">
			<iframe src="<?php echo esc_url($url); ?>" class="venoframe figlio"></iframe>
		</div>		
		<?php } elseif($type=='youtube') {
		$url=MthemeCore::getPostMeta(get_the_ID(),"post_url","Cg4lEyWlm28");		
		?>
		<div class="venoframe">
			<iframe class="html5video" src="http://www.youtube.com/embed/<?php echo esc_attr($url); ?>&rel=0"></iframe>
		</div>		
		...
		<p class="blog-text"><?php echo mtheme_html_content(MthemeCore::getPostMeta(get_the_ID(),"post_overview",$post_content)); ?></p>
