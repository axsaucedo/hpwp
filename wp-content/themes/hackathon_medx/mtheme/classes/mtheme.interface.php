...
class MthemeInterface {
	...
	public static function renderOption($option) {
		...
		/*if($option['type']=='select' && isset($option['default']) && empty($option['value']))
		{
		...
		switch($option['type']) {
			...
			//colorpicker
			case 'color':
				$out.='<input name="'.$option['id'].'" id="'.$option['id'].'" type="text" value="'.$option['value'].'" class="mtheme-colorpicker" />';
			break;
			...
			case 'select_post':
				...
				if(isset($post) && isset($post->ID)) {
					...
					if($option['post_type']=='lesson' && $post->post_type=='lesson') {
						$course=MthemeCore::getPostRelations($post->ID, 0, 'lesson_course', true);
						$atts['include']=array_diff(MthemeCore::getPostRelations(0, $course, 'lesson_course'), array($post->ID));
					}
				}
	...
	public static function renderTemplateTitle($title, $sep) {
		if(isset($_GET['s']) && empty($_GET['s'])) {
			$title=__('Search Results', 'mtheme');
			$title.=' '.$sep.' ';
		}
	
		foreach(MthemeCore::$components['rewrite_rules'] as $key=>$rule) {
