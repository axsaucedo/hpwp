...
class MthemeForm {
     ...
     * @return void
     */
	public static function renderContactForm($slug, $options=array(), $values=array()) {
		$options=wp_parse_args($options, array(
			'edit' => true,
			'before_title' => '',
			'after_title' => '',
	...
	public static function submitData() {
		...
		if(isset($data['slug']) && self::isActive($data['slug'])) {			
			...
			if(!empty(MthemeInterface::$messages)) {
				...
				if(mtheme_mail($email, $subject, $message) && isset(self::$data[$data['slug']]['message'])) {
